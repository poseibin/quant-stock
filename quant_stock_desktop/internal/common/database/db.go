...
func baseSchemaStatements() []string {
	return []string{
		...
		`CREATE TABLE IF NOT EXISTS strategy_observation_pool (
			...
			last_rank_pct REAL NOT NULL DEFAULT 0,
			best_rank_pct REAL NOT NULL DEFAULT 0,
			status TEXT NOT NULL DEFAULT 'active',
			enter_reason TEXT NOT NULL DEFAULT '',
			keep_reason TEXT NOT NULL DEFAULT '',
			exit_reason TEXT NOT NULL DEFAULT '',
			payload_json TEXT NOT NULL DEFAULT '{}',
			created_at TEXT NOT NULL,
			updated_at TEXT NOT NULL,
			PRIMARY KEY(strategy, ts_code)
		);`,
		`CREATE INDEX IF NOT EXISTS idx_strategy_observation_pool_status ON strategy_observation_pool(strategy, status, last_seen_date);`,
		`CREATE TABLE IF NOT EXISTS strategy_observation_events (
			id TEXT PRIMARY KEY,
			strategy TEXT NOT NULL,
			run_id TEXT NOT NULL,
			trade_date TEXT NOT NULL,
			...
			rank_no INTEGER NOT NULL DEFAULT 0,
			score REAL NOT NULL DEFAULT 0,
			rank_pct REAL NOT NULL DEFAULT 0,
			reason TEXT NOT NULL DEFAULT '',
			payload_json TEXT NOT NULL DEFAULT '{}',
			created_at TEXT NOT NULL
		);`,
		`CREATE INDEX IF NOT EXISTS idx_strategy_observation_events_code ON strategy_observation_events(strategy, ts_code, trade_date);`,
